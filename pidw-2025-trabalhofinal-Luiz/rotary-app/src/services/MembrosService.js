...
class MembrosService {
  ...
  async listarTodos() {
    try {
      ...
      const membrosFormatados = dados.map((m) => {
        ...
        console.log("Membro formatado:", membro);
        return membro;
      });

      console.log("Membros finais:", membrosFormatados);
  ...
  async cadastrar(membro) {
    ...
    if (membroExistente) {
      ...
      console.log("Enviando para API:", membroFormatado);
  ...
  async atualizar(cpf, membro) {
    try {
      ...
      const cpfLimpo = cpf.replace(/[.-]/g, "");
      console.log("Atualizando membro:", cpfLimpo, membroFormatado);
